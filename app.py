...
def api_admin_testar_op(ordem):
    """Permite ao admin conferir se uma OP específica está no banco."""
    db = Session()
    try:
        alvo = _norm_ordem(ordem)
        item = db.query(ItemMestre).filter_by(ordem=alvo).first()
        if item:
            return jsonify({'encontrado': True, **item.to_dict()})
        return jsonify({'encontrado': False, 'ordem': alvo})
    finally:
        db.close()


def _norm_ordem(v):
    """Normaliza a OP: tira espaços e o '.0' que o Excel às vezes coloca."""
    s = str(v).strip()
    if s.endswith('.0'):
        s = s[:-2]
    return s
...
def importar_mestre(db, linhas):
    ...
    Otimizado para grandes volumes (ex: 17 mil ordens): 1 consulta + lotes.
    """
    # carrega todas as ordens já existentes de uma vez (1 query, não N)
    existentes = {o.ordem: o for o in db.query(ItemMestre).all()}
    novos = atual = 0
    novos_objs = []
    vistos = set()  # evita duplicar dentro do mesmo arquivo

    for i, row in enumerate(linhas):
        if not row or all(c is None or str(c).strip() == '' for c in row):
            continue
        c0 = str(row[0]).strip().lower()
        if 'ordem' in c0 or c0 in ('order', 'ordem'):  # cabeçalho
            continue
