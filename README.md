...
3. Durante a preparação, pode **Pausar** (café, ginástica laboral) — o tempo
   parado **não conta**; é só tocar em **Retomar** para voltar. O cesto fica
   marcado como "Pausado".
4. Ao terminar de encher o cesto, toca em **Parar tempo** — o cronômetro
   congela e registra o tempo de preparação (já descontando as pausas).
5. **Depois**, com calma, preenche os dados: **bipa o código de barras da OP**
   (preenche código/material, texto breve e quantidade da lista mestra — tudo
   editável), escolhe processo, tipo e observação, e conclui. O cesto **vai para
   a fila do banho**.
6. **Operador de banho** vê a fila, inicia o banho (novo cronômetro) e finaliza
   quando o cesto sai. Aí o cesto **volta a ficar livre** na grade.
7. Qualquer card pode ser **editado** depois (tocar no cesto ocupado) para
   corrigir informação errada.

A pausa existe apenas na preparação (pré-banho). Todas as telas atualizam
sozinhas a cada poucos segundos.
